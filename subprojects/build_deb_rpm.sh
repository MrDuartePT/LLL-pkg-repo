...
#Change version according to tag
sed -i "s/%define version _VERSION/%define version $TAG/g" darkdetect.spec
sed -i "s/%define unmangled_version _VERSION/%define unmangled_version $TAG/g" darkdetect.spec
#
...
#Move to repo
cd ${REPODIR}/ubuntu
cp ${BUILD_DIR}/python3-darkdetect_$TAG-1_all.deb
